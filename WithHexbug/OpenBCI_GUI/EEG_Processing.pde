...
class EEG_Processing_User {
  ...
  final float detection_thresh_dB = 7.0f; //how much bigger must the peak be relative to the background

  //add your own variables here
  final float min_allowed_peak_freq_Hz = 5.0f; //was 4.0f, input, for peak frequency detection
  final float max_allowed_peak_freq_Hz = 18.0f; //was 15.0f, input, for peak frequency detection
  final float[] processing_band_low_Hz = {6, 9, 10, 14}; //lower bound for each frequency band of interest (2D classifier only) // 7.7, 10.5, 9, 13.5
  final float[] processing_band_high_Hz = {8, 9, 12, 16};  //upper bound for each frequency band of interest // 8.4, 11.5, 10, 16.5
  public DetectedPeak[] detectedPeak;  //output per channel, from peak frequency detection
  public DetectedPeak[] peakPerBand;
  int[] bandDetectionCount;  // keep track of how many times peak was detected at a processing band 0, 1, 2, 3
  int[] bandFirstDetectionTime;  // millis when a peak was last detected
  int peakHoldTime = 3 * 1000;  // ms duration to check for continuous peak detection i.e. a peak has to be held for x seconds to be considered a command
  int detectionCountThresh = 10;  // how many times a peak must be detected within peakHoldTime to count as a command sent
  HexBug hexBug;
  ...
  public boolean peakDetected = false;
  ...
  EEG_Processing_User(int NCHAN, float sample_rate_Hz, HexBug hBug) {
    ...
    bandDetectionCount = new int[nBands];
    for (int Iband=0; Iband<nBands; Iband++) bandDetectionCount[Iband] = 0;  // set all counts to 0
    
    bandFirstDetectionTime = new int[nBands];
    for (int Iband=0; Iband<nBands; Iband++) bandFirstDetectionTime[Iband] = 0;
  }
  ...
  FFT[] fftData) {              //holds the FFT (frequency spectrum) of the latest data
    ...
    commandTxt = "";
    peakDetected = false;
    int Ichan = (7);      //change according to the channel being used for corresponding action
    findPeakFrequency(fftData, Ichan);
    if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[3-2]) && (detectedPeak[Ichan].freq_Hz < processing_band_high_Hz[3-2])) { //look in alpha band
      ...
      if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[3-1]) && (detectedPeak[Ichan].freq_Hz < processing_band_high_Hz[3-1])) { //look in alpha band
        ...
        Ichan = (1);    //change according to the channel being used for corresponding action
        findPeakFrequency(fftData, Ichan);
        if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[3-2]) && (detectedPeak[Ichan].freq_Hz < processing_band_high_Hz[3-2])) {
          if (detectedPeak[Ichan].SNR_dB >= detection_thresh_dB) {
            detectedPeak[Ichan].threshold_dB = detection_thresh_dB;
            detectedPeak[Ichan].isDetected = true;
            hexBug.left(); 
          ...
          Ichan = (1);    //change according to the channel being used for corresponding action
          findPeakFrequency(fftData, Ichan);
          if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[3-3]) && (detectedPeak[Ichan].freq_Hz < processing_band_high_Hz[3-3])) {
            if (detectedPeak[Ichan].SNR_dB >= detection_thresh_dB) {
              detectedPeak[Ichan].threshold_dB = detection_thresh_dB;
              detectedPeak[Ichan].isDetected = true;
              hexBug.right(); 
    ...
    if (isDetected) {
      ...
      commandTxt = txt;
      //print some output
      println("EEG_Processing_User: " + "At time: " + int(float(currentTableRowIndex)/openBCI.get_fs_Hz()) + " sec ," + " Command: " + txt + "!, Chan " + (Ichan+1) + ", peak = " + detectedPeak[Ichan].rms_uV_perBin + " uV at " 
        + detectedPeak[Ichan].freq_Hz + " Hz with background aty = " + detectedPeak[Ichan].background_rms_uV_perBin 
  ...
  FFT[] fftData) {              //holds the FFT (frequency spectrum) of the latest data
    ...
    int Ichan = 5;  //which channel to act on. Select channel user is hooked to. 5 in code = 6 on board
    ...
    if (detectedPeak[Ichan].SNR_dB >= detection_thresh_dB) {
      ...
      if ((detectedPeak[Ichan].freq_Hz > processing_band_low_Hz[0]) && (detectedPeak[Ichan].freq_Hz < processing_band_high_Hz[0])) {
        // turn arm left/right
        /*
        bandDetectionCount[0]++;
        println("bandDetectionCount[0] = " + bandDetectionCount[0]);
        if((millis() - bandFirstDetectionTime[0]) >= peakHoldTime) {  // once peakHoldTime duration has elapsed
          
          if(bandDetectionCount[0] >= detectionCountThresh) {
            hexBug.forward();
            txt = "FWD";
            isDetected = true;
          } else {  // too few detections, so reject the command and reset the count
            bandDetectionCount[0] = 0;
            bandFirstDetectionTime[0] = millis();
          }
          
        } // end check peakHoldTime 
        
        */
        
      } else if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[1]) && (detectedPeak[Ichan].freq_Hz <= processing_band_high_Hz[1])) {
        hexBug.reverse(); 
        txt = "RVS";
        isDetected = true;
      }else if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[2]) && (detectedPeak[Ichan].freq_Hz <= processing_band_high_Hz[2])) {
        // move arm up/down
        bandDetectionCount[2]++;
        println("bandDetectionCount[2] = " + bandDetectionCount[2]);
        if((millis() - bandFirstDetectionTime[2]) >= peakHoldTime) {  // once peakHoldTime duration has elapsed
          
          if(bandDetectionCount[2] >= detectionCountThresh) {
            hexBug.left();
            txt = "LFT";
            isDetected = true;
          } else {  // too few detections, so reject the command and reset the count
            bandDetectionCount[2] = 1;
            bandFirstDetectionTime[2] = millis();
          }
          
        } // end check peakHoldTime  

      }else if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[3]) && (detectedPeak[Ichan].freq_Hz <= processing_band_high_Hz[3])) {
        // open/close claw
        bandDetectionCount[3]++;
        println("bandDetectionCount[3] = " + bandDetectionCount[3]);
        if((millis() - bandFirstDetectionTime[3]) >= peakHoldTime) {  // once peakHoldTime duration has elapsed
          
          if(bandDetectionCount[3] >= detectionCountThresh) {
            hexBug.right();
            txt = "RGT";
            isDetected = true;
          } else {  // too few detections, so reject the command and reset the count
            bandDetectionCount[3] = 1;
            bandFirstDetectionTime[3] = millis();
          }
      ...
      if(isDetected) {
        ...
        {
      //print some output
      println("EEG_Processing_User: " + "At time: " + int((float(fileoutput.getRowsWritten()))/openBCI.get_fs_Hz()) + " sec ," + " Command: " + txt + "!, Chan " + (Ichan+1) + ", peak = " + detectedPeak[Ichan].rms_uV_perBin + " uV at " 
        + detectedPeak[Ichan].freq_Hz + " Hz with background atx = " + detectedPeak[Ichan].background_rms_uV_perBin 
        + ", SNR (dB) = " + detectedPeak[Ichan].SNR_dB);
        }
