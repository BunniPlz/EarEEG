...
class DataProcessing_User {
  ...
  public DetectedPeak[] detectedPeak;
  public DetectedPeak[] peakPerBand;
  public boolean peakDetected = false;
  boolean switchesActive = false;
  final float detection_thresh_dB = 7.0f;
  final float min_allowed_peak_freq_Hz = 5.0f;
  final float max_allowed_peak_freq_Hz = 21.0f;
  final float[] processing_band_low_Hz = {5.0f};
  final float[] processing_band_high_Hz = {21.0f};
     ...
     */
      ...
      if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[0]) && (detectedPeak[Ichan].freq_Hz < processing_band_high_Hz[0])) {
        if (detectedPeak[Ichan].SNR_dB >= detection_thresh_dB) {
          ...
          detectedPeak[Ichan].isDetected = true;
          isDetected = true;
          println("Detected peak at channel 6. Frequency: " + detectedPeak[Ichan].freq_Hz + " SNR: " + detectedPeak[Ichan].SNR_dB);
        }
      }
      ...
      if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[0]) && (detectedPeak[Ichan].freq_Hz < processing_band_high_Hz[0])) {
        if (detectedPeak[Ichan].SNR_dB >= detection_thresh_dB) {
          ...
          detectedPeak[Ichan].isDetected = true;
          isDetected = true;
          println("Detected peak at channel 5. Frequency: " + detectedPeak[Ichan].freq_Hz + " SNR: " + detectedPeak[Ichan].SNR_dB);
        }
      }
      
      
      peakDetected = isDetected;
  }
