...
public class OutputFile_rawtxt {
  ...
  public void writeRawData_dataPacket(DataPacket_ADS1299 data, float scale_to_uV, float scale_for_aux, int stopByte) {
    ...
    if (output != null) {
      ...
      // write SNR info for detected peaks
      if(dataProcessing_user.peakDetected) {
        int detectedPeak_length = dataProcessing_user.detectedPeak.length;
        //println("detectedPeak_length = " + detectedPeak_length);
        for(int i=0; i < detectedPeak_length; i++) {
          float snr = dataProcessing_user.detectedPeak[i].SNR_dB;
          float frequency = dataProcessing_user.detectedPeak[i].freq_Hz;
          if(snr > 0) {
            output.print( "," + snr);
